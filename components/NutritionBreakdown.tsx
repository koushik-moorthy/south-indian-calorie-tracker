...
export default function NutritionBreakdown({
  ...
  const macros = NUTRIENT_FIELDS.filter(
    (f) => f.group === "macro" && nutrition[f.key] != null
  );
  const micros = NUTRIENT_FIELDS.filter(
    (f) => f.group === "micro" && nutrition[f.key] != null
  );

  function Grid({ fields }: { fields: typeof NUTRIENT_FIELDS }) {
    return (
