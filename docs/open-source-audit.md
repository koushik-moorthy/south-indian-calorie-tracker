...
| Real Supabase anon key / `SETTINGS_ENC_KEY` value | **None** | history scan for `9pPbz7x3y…` → not present; `.env.local` is gitignored (`.gitignore` `.env*.local`) |
| Supabase project ref (`dpcktewfvnqqprfzthyt`) | **Not present** anywhere tracked | grep tree + history → none |
